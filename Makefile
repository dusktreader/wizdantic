PACKAGE_TARGET:=src/wizdantic src/wizdantic_demo
...
qa/types:  ## Run static type checks
	@uv run --extra=demo ty check ${PACKAGE_TARGET} tests
...
demo/run:  ## Run the demo application
	@uv run --extra=demo wizdantic-demo

demo/debug:  ## Run the demo application in debug mode
	@uv run --extra=demo debugpy --listen localhost:5678 --wait-for-client wizdantic-demo
